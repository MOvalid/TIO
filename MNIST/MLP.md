...
* **Cel:** inicjalizacja sieci
* **Parametry:**

  * `input_shape` – liczba cech wejściowych (784 dla MNIST)
  * `hidden_units` – lista liczby neuronów w warstwach ukrytych
  * `activation` – funkcja aktywacji w warstwach ukrytych (ReLU/Tanh)
  * `dropout_rate` – opcjonalny dropout dla regularizacji
  * `num_classes` – liczba klas wyjściowych
  * `learning_rate` – szybkość uczenia
* Na końcu wywołujemy `_initialize_weights()`, aby losowo ustawić wagi i biasy dla wszystkich warstw

---

## 2. Inicjalizacja wag `_initialize_weights`
...
* Tworzymy listę rozmiarów warstw: wejście → ukryte → wyjście
* **Wagi** (`weight_matrix`) losujemy z rozkładu normalnego, skalując `np.sqrt(2/n_in)` (He initialization dla ReLU)
* **Biasy** (`bias_vector`) inicjalizowane zerami
* Zwracamy listy `weights_list` i `biases_list`, które będziemy aktualizować podczas uczenia
...
* `_relu(x)` – zwraca `max(0, x)`
* `_relu_derivative(x)` – pochodna ReLU (potrzebna w backward pass)
* `_softmax(x)` – zamienia wyjścia warstwy na **prawdopodobieństwa klas**

---

## 4. Forward pass `_forward`
...
* Przechodzimy po wszystkich warstwach ukrytych:

  * `z = a_prev @ W + b` → liniowa kombinacja wejścia
  * `a = ReLU(z)` → aktywacja
  * zapisujemy `z` i `a` do list
* Warstwa wyjściowa:

  * `z = a_prev @ W + b`
  * `a = softmax(z)` → prawdopodobieństwa
* Zwracamy:

  * `activations_list` – lista aktywacji wszystkich warstw
  * `linear_combinations` – lista liniowych kombinacji (potrzebne do backward pass)
...
```python
def _backward(self, X_batch, Y_batch, activations_list, linear_combinations):
    batch_size = X_batch.shape[0]
    weight_gradients = [np.zeros_like(w) for w in self.weights]
    bias_gradients = [np.zeros_like(b) for b in self.biases]

    # Output layer
    delta_output = (activations_list[-1] - Y_batch) / batch_size
...
* **Delta w warstwie wyjściowej:** `delta_output = (y_pred - y_true)/batch_size`
* **Gradienty wag i biasów:** `weight_gradients = a_prev.T @ delta`, `bias_gradients = sum(delta)`
* **Propagacja wstecz przez ukryte warstwy:** `delta = delta_next @ W_next.T * ReLU'(z)`
* **Aktualizacja wag i biasów:** `W -= learning_rate * weight_gradients`, `b -= learning_rate * bias_gradients`
...
* Losowa permutacja danych (shuffle) dla SGD
* Forward + backward dla każdego batcha
* Obliczenie **accuracy** dla treningu i walidacji
* Zapis do historii uczenia (`history`)
...
* Oblicza **cross-entropy loss** i **accuracy**
* Użyteczne do testów na zbiorze walidacyjnym/testowym
